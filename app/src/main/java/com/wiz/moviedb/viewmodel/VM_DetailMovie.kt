...
data class DataReviews(var reviewList : ReviewlistModel, var networkState: NetworkState)

class VM_DetailMovie(private val context: Context): ViewModel() {
    ...
    class VMF_DetailMovie(private val context: Context): ViewModelProvider.NewInstanceFactory() {
        override fun <T : ViewModel?> create(modelClass: Class<T>): T = VM_DetailMovie(context) as T
    }
}